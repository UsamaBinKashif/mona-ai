...
const Chat = ({ roomId, setChatBox }: Props) => {
  ...
  const sendMessage = (
    ...
    if (msg && isUserLoggedIn) {
      socket.emit('BE-send-message', { roomId, msg, sender: userDisplayName }); // Use the user's first name
      if (inputRef.current) {
  ...
  const stopRecording = async () => {
    try {
      ...
      socket.emit('BE-send-audio', {
        ...
        sender: userDisplayName, // Name of the sender (e.g., user's name)
      });
            ...
            <div>
              <>
                {msg
                  ...
                  .map(
                    (
                      { sender, msg, fileURL, timestamp }: any,
                      index: number,
                    ) => (
                      <div
